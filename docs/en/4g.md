...
> **Info** A Raspberry Pi camera device `/dev/video0` could be used by `drona` systemd service. In this case `gst-rtsp-launch` will not get an access to the device. For stop the `drona` run command `sudo systemctl stop drona`.
