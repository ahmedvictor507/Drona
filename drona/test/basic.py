...
@pytest.fixture()
def node():
    return rospy.init_node('drona_test', anonymous=True)
...
def test_blocks(node):
    rospy.wait_for_service('drona_blocks/run', timeout=5)
    rospy.wait_for_service('drona_blocks/stop', timeout=5)
    rospy.wait_for_service('drona_blocks/load', timeout=5)
    rospy.wait_for_service('drona_blocks/store', timeout=5)

    from std_msgs.msg import String
    from drona_blocks.srv import Run

    def wait_print():
        try:
            wait_print.result = rospy.wait_for_message('drona_blocks/print', String, timeout=5).data
    ...
    run = rospy.ServiceProxy('drona_blocks/run', Run)
