Controlling Drona from a smartphone
===
...
To control Drona from a smartphone via Wi-Fi, you have to install the appropriate application – [iOS](https://itunes.apple.com/ru/app/clever-rc/id1396166572?mt=8), Android (https://play.google.com/store/apps/details?id=express.copter.cleverrc).
...
Install [Drona image on RPi](image.md). For running the application, settings `rosbridge` and `rc` in the launch file (`~/catkin_ws/src/drona/drona/launch/drona.launch`) should be enabled:
...
After the launch-file is edited, restart package `drona`:
...
Connect the smartphone to Drona [Wi-Fi](wifi.md) network (`drona-xxxx`). The application should connect to the copter automatically. Upon successful connection, the current [mode](modes.md) and the battery charge level should be displayed.
