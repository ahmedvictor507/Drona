...
Basic information on working with SSH can be found in the section [SSH access to Raspberry Pi](ssh.md). In this section you will find advanced information on using SSH, as well as a number of recommendations on using SSH when working with Drona.
...
SSH allows you not only to organize work in the command shell, but also to transfer files, as well as tunnel transmitted data from other protocols, such as information from a video camera or telemetry. In addition, SSH supports several authentication modes (that is, verification of the connecting user), with its help it is possible to connect to the Drona not only using a password, but also password-free access (authentication by a key pair, i.e. SSH keys).

## Password authentication

Authentication [by password](ssh.md) on the image of RPi for Drona is enabled by default and the password can be used to enter into the command shell of the minicomputer. On computers with Linux operating systems (and primarily on servers connected to the Internet), the ability to login with a password is usually disabled, since there is a more secure authentication method.

> **Hint** It is not recommended to disable logging into Drona by password, since you can completely lose access to the command shell over the network.
...
# Linux drona-3270 5.10.17-v7l+ #1414 SMP Fri Apr 30 13:20:47 BST 2021 armv7l
...
```bash
ssh-keygen -f ~/.ssh/id_drona -C "SSH key for Drona" -N ""
# Your identification has been saved in /home/galina/.ssh/id_drona
# Your public key has been saved in /home/galina/.ssh/id_drona.pub

chmod 400 ~/.ssh/id_drona*
```
...
```bash
ssh-copy-id -i ~/.ssh/id_drona.pub pi@192.168.11.1
# pi@192.168.11.1's password: *********
```
...
```bash
ssh -i ~/.ssh/id_drona pi@192.168.11.1
```
...
## Configuring SSH connection to Drona
...
* the private key `~/.ssh/id_drona` will be used automatically;
...
Now, to connect to RPi from a Linux terminal, you just need to type `ssh 1[TAB][TAB][ENTER]` and the ip address `192.168.11.1` will be automatically updated on the command line, because the command shell uses addresses from the file `~/.ssh/config` and is able to "guess" your intentions to connect to the Drona. By pressing enter, you will instantly find yourself in the RPi terminal.
