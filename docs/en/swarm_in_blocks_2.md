...
Swarm in Blocks (from it's origin in 2022) was born looking to make a *high-level interface based on the blocks language*, to make simple handling swarms, without requiring advanced knowledge in all the necessary platforms, creating tools to allow a lot of applications based on the user needs and also using the Drona platform.
...
<p align="center">
    <img width="600" src="https://raw.githubusercontent.com/Grupo-SEMEAR-USP/swarm_in_blocks/master/assets/intro/dronas_leds.gif" />
</p>
...
For more information on our project from last year, see our final article in [Swarm in Blocks 2022](https://drona.coex.tech/en/swarm_in_blocks.html). In addition, we also recommend watching our final video from last year, [Swarm in Blocks 2022 - Final Video](https://www.youtube.com/watch?v=5C-1rRnyiE8).
...
| Giant clutter to keep track of all Dronas in a swarm individually (several terminals, many simultaneous computers with several people to keep track of)  | Swarm Station  |
| Lack of basic features for handling a swarm pre-implemented in the Drona platform (such as access to battery data and raspberry computational power)  | Full integration of low level data in our Swarm Station  |
...
| Lack of a center for reports of successful tests with swarms of drones for the Drona platform describing the test conditions (odometry, etc.)  | Show off section in our Gitbook  |
...
- *Planning Mode:* Its main goal is to allow the user to check the drones' layout, save and load formations, before starting the simulator or using real dronas. In order to need less computational power and avoid possible errors during the simulation.
...
- *Navigation Mode:* The last mode will support executing everything developed in real dronas so that it's possible to control a swarm with block programming. The biggest obstacle yet is the practical testing of this mode, due to the financial difficulty of acquiring a Drona swarm.
...
In order to fly a real swarm using drona, we decided to take an approach of putting every drona on the same ROS network / environment so that the master could talk to each one of them.

We did this by separating each drone topics / nodes / services with namespaces. The goal is to achieve the same effect as the simulation that we've done in [**CopterHack-2022**](copterhack2022.md), so each drone would have its own `/dronaID` namespace, and the ID is the identifier for each drone.

In other words, instead of just `simple_offboard` node for a single drone, we'd now have `/drona0/simple_offboard`, `/drona1/simple_offboard` and so on.

To launch it, you need to first stop drona's default daemon, and then connect all Raspberries to the same network. After that, you should connect all their `roscore` to the same IP address (the master's), and then launch the `realDrona.launch` file passing the `ID` argument as a parameter. Again, for more detailed information on how this works, please check out our [gitbook](https://swarm-in-blocks.gitbook.io/swarm-in-blocks/):

```bash
sudo systemctl stop drona
roslaunch rasp_pkg realDrona.launch ID:=0
```
...
With that came the Swarm in Blocks 2.0, which brought with it innovations that optimized the drona's flight control and that could allow for greater emotions in the handling of the drone, in addition to focusing on greater flight safety.
The Swarm in Blocks 2.0 presents new features for this year, such as the Web terminal, First Person View (FPV), Collision Avoidance, Drona UI and Swarm Station.
