...
2. Configure wpa_supplicant to work in the access point mode. Change your `/etc/wpa_supplicant/wpa_supplicant.conf` file to contain the following:
    ...
    where `drona-1234` is the network name and `dronawifi` is the password.
...
sudo dnsmasq --interface=wlan0 --address=/drona/coex/192.168.11.1 --no-daemon --dhcp-range=192.168.11.100,192.168.11.200,12h --no-hosts --filterwin2k --bogus-priv --domain-needed --quiet-dhcp6 --log-queries
...
interface=wlan0
address=/drona/coex/192.168.11.1
