...
Last year at CopterHack 2022, we created a [project](../ru/advanced_drona_simulator.html) that simplified the simulation of Drona, and in 2021, we created a [project](../ru/advanced_drona.html) that simplified the development of products for Drona (IDE and library for writing). The time has come to combine them and achieve unlimited power.

### Project Idea

The idea of the project is to combine DronaIDE and DronaSim (a tool for running Drona simulations). Thus, a platform is planned that allows developing products based on Drona using a simulator and an advanced IDE. The platform will include the following features:

- Add a web interface that allows using DronaSim without touching the command line.
- Work both in the browser (without installing anything) and from CLI.
- Have a course that covers different aspects of drona.
...
Installation process is described in the [project documentation](https://ftl-team.github.io/drona_sim/#/?id=installation).

## Usage

The DronaSim platform offers a seamless workflow for users:
...
2. After launching the simulation, users are presented with DronaSim WebUI that
   ...
   access the DronaSim IDE to solve task.
...
- Custom-patched GZWeb with bug fixes and additional features, including the display of the Drona LED strip.
...
We also have developed a learning course based on DronaSim: [link](https://github.com/FTL-team/DronaSim_course). It currently has the following tasks:

- 1_thesquare - First task of DronaSim course with goal to fly square.
...
- [DronaSim](https://github.com/FTL-team/drona_sim) - tool that manages simulation.
- [DronaSim Basefs](https://github.com/FTL-team/drona_sim_basefs) - container image that is used in simulator.
- [Drona IDE](https://github.com/FTL-team/dronaIDE) - drona ide tools and theia.
- [DronaSim course](https://github.com/FTL-team/DronaSim_course) - course with tasks based on our platform.
...
- Migration to squash fs images, which greatly reduced size of installed DronaSim from 13 gigabytes to just 3.5 gigabytes.
- Tasks are now mounted instead of being copied and also build before starting.

Because of the way catkin_make works, it is incredibly slow when new packages are added (whole cmake configuration is rerun for all packages). catkin_make provides a way to build only some packages, but it caches this packages and to reset this cache you need to recompile whole catkin_make. But we have found a solution: `catkin_make -DCATKIN_WHITELIST_PACKAGES="task;DronaSim" --build build_DronaSim` This command, builds only DronaSim and task package in separate build directory, this drastically reduces time that catkin_make takes, and keeps expected behavior of catkin_make without arguments.

There are also differences in tool that launches simulation:

- Client-server architecture allows us to create web UI and run DronaSim on server.
- More robust error handling improves user experience.
- Rewritten in rust, better reliability and development experience.

### DronaSim basefs

Version 2 integrates DronaIDE into system. We also updated drona in simulator to v0.23 and added web terminal. Basefs is now squashed and doesn't require additional installation. It also uses patched(by us) version of gzweb that is more suitable for our use-case:
...
- Drona LED strip is rendered, our gzweb connects to ROS and pulls LED data from there to render LED strip like Gazebo does.
...
DronaIDE got some updates too:

- We have updated theia and extensions used.
- Better C++ support via clangd.
- Drona IDE tools can now reconnect after simulator restart.
- Copter Status now displays LED strip status.
- Tools ui has better support for different themes.

But the most important change is DronaSim integration, there are new tools (task description, simulator control and gzweb). While gzweb tool is just an iframe (though it's very cool to have it in IDE).

Task description and simulator control are more interesting as they have to interact with both IDE and DronaSim, to maintain different versions support we use quite interesting trick, extension webview after being initialized dynamically loads JavaScript from DronaSim. That provides better integration between two.

### DronaSim course

DronaSim course is a new part of our platform. It uses robust task API of DronaSim to create practical learning course. It currently teaches different aspects of drona development that i encountered during my participation in different contests involving drona. But we are happy to accpet suggestions about other aspects we should teach in out course.

## Conclusion

This project is a final (or maybe there is more?) project of our advanced drona saga. AdvancedDrona is a project that is easy to use and greatly improves experience during learning about drona, participating in drona based competitions and development drona based projects. We thank COEX team for their support and look forward to further cooperation.
