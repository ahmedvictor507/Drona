# `drona_simulation` ROS package

This package provides resources necessary for launching Gazebo simulation with Drona, along with `.launch` files for convenience.
...
* `vehicle` (*string*, default: *drona*) - PX4 vehicle name. Depending on this parameter, different PX4 presets will be loaded.
...
This will start a new Gazebo instance (using `gazebo_ros` package), load a PX4 SITL instance, spawn a Drona model and start Drona ROS nodes. The PX4 console will be accessible in the terminal where `roslaunch` was performed.
...
By default, the `simulator.launch` file will start the simulation with [`resources/worlds/drona.world`](resources/worlds/drona.world) as its base world. Note that the `real_time_update_rate` is set to 250 - this is required for PX4 lockstep simulation to work correctly.

If you wish to create your own world for the simulation, be sure to derive it from `drona.world` to avoid issues with PX4 plugins.
...
`simulator.launch` utilizes the same `drona.launch` file from the `drona` ROS package, so ROS node reconfiguration is the same as on the real drone.
...
A visual Gazebo plugin is used for the LED strip. An example of the plugin usage is provided in [`led_strip.xacro`](../drona_description/urdf/leds/led_strip.xacro).
